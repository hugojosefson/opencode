...
When `compaction.mode` is `suffix`, setting `output.prompt` uses prepend mode for that compaction so the replacement semantics remain unchanged. Values added to `output.context` are included in either mode.
